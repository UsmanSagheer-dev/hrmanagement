  ...
  return (
    <div className="bg-transparent h-[380px] text-white rounded-lg p-4 shadow-lg border border-[#A2A1A833] overflow-y-scroll scrollbar-hide">
      <div className="flex justify-between items-center mb-4">
        ...
        <p className="text-[#A2A1A8] text-sm font-light ">{totalMembers} Members</p>
        </div>
        ...
        {members.map((member) => (
          <li key={member.id} className="flex items-center space-x-4">
            ...
            <div>
              ...
              <p className="text-[#A2A1A8] font-light text-[12px]">{member.title}</p>
            </div>
            <span className="ml-auto text-white"><MdArrowForwardIos /></span>
          </li>
        ))}
      </ul>
    </div>
